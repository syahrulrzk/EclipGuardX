...
const ContainerCPUChart = memo(() => {
  ...
  useEffect(() => {
    const fetchContainerCPUData = async () => {
      try {
        setLoading(true)
        setError(null)
        ...
        allMetricsResults.forEach(({ container, metrics }) => {
          ...
          metrics.forEach(metric => {
            ...
            // Only add CPU usage
            timeData[containerName] = Math.round(metric.cpuUsage)
          })
        })
        ...
        const finalData = sortedData.reverse() // Reverse to show latest first
        setChartData(finalData)
      } catch (error) {
        console.error('Error fetching container CPU data:', error)
        setError('Failed to load container CPU metrics')
      } finally {
        setLoading(false)
      }
    }
    ...
    const interval = setInterval(fetchContainerCPUData, 5000)
  ...
  const getContainerColor = (containerName) => {
    const colors = ['#a855f7', '#22c55e', '#06b6d4', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899']
  ...
  return (
    <Card className="bg-gray-800 border-gray-700 text-white">
      <CardHeader>
        ...
        <CardDescription className="text-gray-400">
          Container CPU utilization over time (multiple containers)
        </CardDescription>
      </CardHeader>
      <CardContent>
